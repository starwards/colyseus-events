...
⚠️ It is not recommended to rely on `StateView` for big datasets, as it is not optimized for that yet. However, it is a great way to filter data per client, such as "private fields" per schema instance, "level of detail", area-based, team-owned data, and etc.
...
*Source: https://docs.colyseus.io/state/view*
