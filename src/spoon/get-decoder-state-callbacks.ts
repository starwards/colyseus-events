...
export function getDecoderStateCallbacks(decoder: Decoder): SchemaCallbackProxy {
    ...
    decoder.triggerChanges = function (allChanges: DataChange[]) {
        ...
        for (let i = 0, l = allChanges.length; i < l; i++) {
            ...
            //
            if ((change.op & OPERATION.DELETE) === OPERATION.DELETE && change.previousValue instanceof Schema) {
                const refId = $root.refIds.get(change.previousValue);
                if (refId !== undefined) {
                    const deleteCallbacks = callbacks[refId]?.[OPERATION.DELETE];
        ...
        if (metadata && !isCollection) {
            const onAddListen = function (
                ...
                if (
                    ...
                    (!currentOnAddCallback || !onAddCalls.has(currentOnAddCallback)) // Workaround for https://github.com/colyseus/schema/issues/147
                ) {
                    callback(context.instance[prop], undefined);
                }
                const refId = $root.refIds.get(ref);
                if (refId === undefined) {
                    throw new Error('refId not found');
                }
                return $root.addCallback(refId, prop, callback);
            };
            ...
            return new Proxy(
                {
                    ...
                    bindTo: function bindTo(targetObject: any, properties?: string[]) {
                        ...
                        return $root.addCallback(refId, OPERATION.REPLACE, () => {
            ...
            const onAdd = function (ref: Ref, callback: (value: any, key: any) => void, immediate: boolean) {
                ...
                return $root.addCallback(refId, OPERATION.ADD, (value: any, key: any) => {
            ...
            const onRemove = function (ref: Ref, callback: (value: any, key: any) => void) {
                ...
                return $root.addCallback(refId, OPERATION.DELETE, callback);
            };
            ...
            return new Proxy(
                ...
                },
                {
                    ...
                    has(target, prop) {
                        return target[prop as keyof typeof target] !== undefined;
                    },
                    set(_, _1, _2) {
                        throw new Error('not allowed');
                    },
